...
function buildKpiCards(summary: InvestorDashboardSummary | undefined): KpiCard[] {
  if (!summary) return [];
  ...
  return [
    {
      ...
      val: formatCompactCurrency(summary.portfolioValue.amount, currency, { decimals: 0 }),
        ...
        <section className="grid grid-cols-1 gap-6 xl:grid-cols-3 xl:gap-8">
          <div className="xl:col-span-2 overflow-hidden rounded-[22px] border border-ui-border bg-ui-card p-6 shadow-sm md:rounded-[28px] md:p-9 dark:shadow-[0_4px_28px_-12px_rgba(0,0,0,0.35)]">
            <div className="mb-8 flex flex-col justify-between gap-5 sm:mb-10 sm:flex-row sm:items-center">
              <div>
                <h3 className="mb-1 text-lg font-bold text-ui-strong md:text-xl">Capital Deployed</h3>
            ...
            <div className="mt-8 grid grid-cols-1 gap-6 border-t border-ui-divider pt-8 sm:grid-cols-3 sm:gap-8">
              {[
                {
                  label: 'Current Month',
                  val: formatCompactCurrency(capitalDeployed?.currentMonth ?? 0, currency),
                },
                {
                  label: 'Target Gap',
                  val: formatCompactCurrency(capitalDeployed?.targetGap ?? 0, currency),
                },
                {
                  label: 'Completion',
          ...
          <div className="xl:col-span-2 rounded-[22px] border border-ui-border bg-ui-card p-6 shadow-sm md:rounded-[28px] md:p-9 dark:shadow-[0_4px_28px_-12px_rgba(0,0,0,0.35)]">
            ...
            <div className="space-y-1">
              {activities.length > 0 ? (
                activities.map((item) => (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[10px] font-semibold uppercase tracking-wide text-ui-faint">{item.id}</p>
                    </div>
                ...
                <p className="py-8 text-center text-xs font-medium text-ui-muted-text">No recent activity</p>
              )}
            </div>
          </div>
              ...
              {upcomingEvents.length > 0 ? (
                upcomingEvents.map((event) => (
                  ...
                  >
                    ...
                    >
                      <p className="text-[8px] font-bold uppercase leading-none opacity-90">{event.month}</p>
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] font-medium text-ui-muted-text">{event.subtitle}</p>
                    </div>
                  </div>
                ))
              ) : (
                <p className="py-8 text-center text-xs font-medium text-ui-muted-text">No upcoming events</p>
              )}
            </div>
