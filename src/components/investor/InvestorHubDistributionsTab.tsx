    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {[
          ...
          >
            <div className={`mb-6 flex h-10 w-10 items-center justify-center rounded-full ${card.iconBg}`}>
              <card.Icon className="h-5 w-5" strokeWidth={iconStroke} />
            </div>
            <p className="mb-4 text-[9px] font-bold uppercase tracking-widest text-ui-faint">{card.label}</p>
            <p className={card.compact ? 'text-base font-bold text-ui-strong md:text-lg' : 'text-2xl font-bold text-ui-strong'}>
              {card.value}
            </p>
            {card.sub ? <p className="mt-4 text-xs font-medium text-ui-faint">{card.sub}</p> : null}
          </div>
        ))}
      </div>
